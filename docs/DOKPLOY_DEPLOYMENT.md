...
3. Dokploy Domains ile `ozeldersal.net` ve `www.ozeldersal.net` alan adlarını bağlayın; TLS sertifikasını Dokploy/Traefik yönetsin. Uygulama portunu internete doğrudan açmayın.
4. İlk kez mevcut bir veritabanına geçiş yapıyorsanız, şemayı ve yedeği doğruladıktan sonra Dokploy Variables'a geçici olarak `PRISMA_BASELINE_EXISTING_DB=true` ekleyin ve bir kez redeploy edin. Başarılı açılıştan sonra değişkeni silin. Bu, mevcut şemayı başlangıç migration'ına tanıtır ve yeni migration'ları uygular. Boş bir üretim veritabanında bu değişkene gerek yoktur; container başlangıcında `prisma migrate deploy` migration'ı uygular.
...
- Dokploy Domains üzerinden `ozeldersal.net` ve `www.ozeldersal.net` için TLS sertifikasının **Issued/Active** olduğunu kontrol edin; HTTP'yi HTTPS'e yönlendirin.
...
- Brevo hesabında doğrulanmış gönderen alan adı oluşturun ve Dokploy Variables'a `EMAIL_FROM`, `EMAIL_FROM_NAME`, `BREVO_API_KEY` ekleyin. E-posta doğrulama ve şifre sıfırlama bu değişkenler olmadan production'da bilinçli olarak çalışmaz.
...
- Uptime uyarısı için en az 1 dakikada bir `https://www.ozeldersal.net/api/health` kontrolü, merkezi hata takibi için Sentry (veya eşdeğeri), loglar için Dokploy log forwarder/sağlayıcı yapılandırın.
